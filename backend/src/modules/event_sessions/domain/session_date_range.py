...
class SessionDateRange:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.start_at, datetime) or not isinstance(self.end_at, datetime):
            raise InvalidArgumentError("SessionDateRange requires datetime instances")
        if self.start_at >= self.end_at:
            raise InvalidArgumentError("SessionDateRange start_at must be strictly before end_at")

    def overlaps(self, other: "SessionDateRange") -> bool:
